...
export default function NomadFlowLanding() {
  ...
  // NATIVE REACT SCROLL TRACKER - 100% ΑΛΑΝΘΑΣΤΟ
  useEffect(() => {
    const handleScroll = () => {
      const vh = window.innerHeight;

      // 1. Υπολογισμός για το Hero Zoom (h-[150vh])
      if (heroRef.current) {
        const heroRect = heroRef.current.getBoundingClientRect();
        // Πόσο έχουμε σκρολάρει μέσα στο Hero
        const heroScrolled = -heroRect.top;
        const heroTotalScroll = heroRect.height - vh;
        if (heroTotalScroll > 0) {
          const progress = Math.min(1, Math.max(0, heroScrolled / heroTotalScroll));
          setHeroProgress(progress);
        }
      }

      // 2. Υπολογισμός για το Sticky Story (h-[300vh])
      if (storyRef.current) {
        const storyRect = storyRef.current.getBoundingClientRect();
        const storyScrolled = -storyRect.top;
        const storyTotalScroll = storyRect.height - vh;

        if (storyScrolled < 0) {
          setActiveStep(0);
        } else if (storyScrolled >= storyTotalScroll) {
          setActiveStep(2);
        } else {
          // Το progress πηγαίνει από 0 έως 1 όσο κάνουμε scroll μέσα στο section
          const progress = storyScrolled / storyTotalScroll;
          if (progress < 0.33) {
            setActiveStep(0);
    ...
    // Passive listener για τέλειο performance
    window.addEventListener("scroll", handleScroll, { passive: true });
    handleScroll(); // Αρχικός υπολογισμός
    return () => window.removeEventListener("scroll", handleScroll);
  }, []);
  ...
  // Inline styles υπολογισμένα από το heroProgress
  const heroScale = 1.15 - (heroProgress * 0.15); // Από 1.15 πέφτει στο 1.0
  const heroRadius = heroProgress * 40; // Από 0 φτάνει 40px
  const heroTextOpacity = 1 - (heroProgress * 2); // Εξαφανίζεται το κείμενο
  const phoneTranslateY = 100 - (heroProgress * 100); // Ανεβαίνει από το +100% στο 0%
  const phoneOpacity = Math.min(1, heroProgress * 2); // Εμφανίζεται σταδιακά

  return (
    // ΠΡΟΣΟΧΗ: Αφαιρέθηκε το overflow-y-auto για να δουλεύει τέλεια το sticky!
    <main className="bg-white relative min-h-screen font-sans w-full overflow-x-hidden text-gray-900">
      <Navbar />

      {/* --- SECTION 1: HERO ZOOM OUT (h-[150vh]) --- */}
      <section ref={heroRef} className="relative h-[150vh] bg-[#F8F9FA]">
        <div className="sticky top-0 h-screen w-full overflow-hidden flex flex-col items-center justify-center">
          ...
          <div 
            ...
            className="absolute top-[20vh] z-20 text-center max-w-4xl mx-auto px-6 flex flex-col items-center pointer-events-none"
          >
            ...
            <button className="bg-[#FF6B35] text-white px-8 py-4 rounded-full font-bold text-lg pointer-events-auto shadow-[0_10px_20px_rgba(255,107,53,0.3)]">
              Get the app
            </button>
          </div>

          {/* Το κινητό του Hero που ανεβαίνει */}
          <div 
            style={{ transform: `translateY(${phoneTranslateY}%) scale(0.9)`, opacity: phoneOpacity }}
            className="absolute bottom-[-5vh] z-30 w-[280px] md:w-[320px] h-[580px] bg-black rounded-[3rem] border-[8px] border-gray-900 shadow-2xl p-3 flex flex-col pointer-events-none will-change-transform"
          >
            <div className="absolute top-0 inset-x-0 h-6 flex justify-center z-[60]">
              <div className="w-28 h-4 bg-gray-900 rounded-b-2xl"></div>
            </div>
            <div className="relative w-full h-full bg-[#E5E9EC] rounded-[2rem] overflow-hidden pt-10 px-4 pb-4 flex flex-col justify-between">
               ...
               <div className="relative z-10 bg-white/95 backdrop-blur rounded-2xl p-3.5 shadow-md flex items-center gap-3">
                 <div className="w-8 h-8 rounded-full bg-[#FF6B35] text-white flex items-center justify-center font-bold text-sm">M</div>
      ...
      {/* --- SECTION 2: INTERACTIVE STICKY PHONE SCROLL STORY (h-[300vh]) --- */}
      {/* Εδώ το scroll «κλειδώνει» επειδή το section έχει ύψος 300vh και το εσωτερικό div είναι sticky h-screen */}
        ...
        <div className="sticky top-0 h-screen w-full flex items-center justify-center overflow-hidden">
          <div className="max-w-7xl mx-auto px-6 w-full flex flex-col md:flex-row items-center justify-between gap-12">
            ...
            <div className="flex-1 text-center md:text-left mb-6 md:mb-0 relative h-[250px] md:h-[350px]">
              ...
              {/* Κείμενο Step 0 (Plan) */}
              <div className={`absolute inset-0 w-full transition-all duration-700 flex flex-col justify-center ${activeStep === 0 ? 'opacity-100 translate-y-0 z-10' : 'opacity-0 translate-y-4 pointer-events-none z-0'}`}>
                ...
                <div className="flex gap-1.5 mt-6 justify-center md:justify-start">
                  <span className="w-6 h-1.5 bg-[#FF6B35] rounded-full transition-all duration-300"></span>
                  <span className="w-2 h-1.5 bg-gray-300 rounded-full transition-all duration-300"></span>
                  <span className="w-2 h-1.5 bg-gray-300 rounded-full transition-all duration-300"></span>
                </div>
              </div>

              {/* Κείμενο Step 1 (Track) */}
              <div className={`absolute inset-0 w-full transition-all duration-700 flex flex-col justify-center ${activeStep === 1 ? 'opacity-100 translate-y-0 z-10' : 'opacity-0 translate-y-4 pointer-events-none z-0'}`}>
              ...
              {/* Κείμενο Step 2 (Relive) */}
              <div className={`absolute inset-0 w-full transition-all duration-700 flex flex-col justify-center ${activeStep === 2 ? 'opacity-100 translate-y-0 z-10' : 'opacity-0 translate-y-4 pointer-events-none z-0'}`}>
            ...
            <div className="flex-1 flex justify-center z-20 relative">
              <div className="relative w-[280px] md:w-[320px] h-[560px] md:h-[620px] bg-black rounded-[3rem] border-[8px] border-gray-900 shadow-2xl p-3 flex flex-col text-gray-900">
                <div className="absolute top-0 inset-x-0 h-6 flex justify-center z-[60] pointer-events-none">
                  <div className="w-28 h-4 bg-gray-900 rounded-b-2xl"></div>
                </div>

                <div className="relative w-full h-full bg-[#f4f6f8] rounded-[2rem] overflow-hidden">
                  
                  {/* --- SCREEN 0: MAP VIEW --- */}
                  <div className={`absolute inset-0 w-full h-full bg-[#E5E9EC] transition-all duration-500 p-4 pt-10 flex flex-col justify-between ${activeStep === 0 ? "opacity-100 z-30" : "opacity-0 pointer-events-none z-0"}`}>
                  ...
                  {/* --- SCREEN 1 & 2: FEED + CARD VIEW --- */}
                  <div className={`absolute inset-0 w-full h-full bg-[#f4f6f8] p-3 pt-8 flex flex-col gap-3 transition-all duration-500 ${activeStep >= 1 ? "opacity-100 z-40" : "opacity-0 pointer-events-none"}`}>
                    
                    {/* Feed Rows (Slide down/up based on step) */}
                    <div className={`flex flex-col gap-2 transition-all duration-700 transform origin-top ${activeStep === 1 ? "scale-y-100 opacity-100 h-auto" : "scale-y-0 opacity-0 h-0 overflow-hidden"}`}>
                      <div className="bg-white rounded-2xl p-2.5 flex items-center gap-3 shadow-sm border border-gray-100">
                      ...
                      <div className="bg-white rounded-2xl p-2.5 flex items-center gap-3 shadow-sm border border-gray-100">
                        <div className="w-10 h-10 bg-gray-200 rounded-xl bg-cover bg-center" style={{ backgroundImage: `url('https://images.unsplash.com/photo-1469854523086-cc02fe5d8800?w=100')` }}></div>
                        <div className="flex-1 text-left"><p className="text-[11px] font-bold">Patagonia road trip</p><div className="h-1.5 w-24 bg-gray-100 rounded mt-1"></div></div>
                      </div>
                    </div>

                    {/* Main Animated Master Card */}
                    <div className={`bg-white rounded-[2rem] shadow-lg border border-gray-100 overflow-hidden flex flex-col flex-1 transition-all duration-700 transform ${activeStep >= 1 ? "translate-y-0 opacity-100" : "translate-y-4 opacity-0"}`}>
                      
                      {/* Card Cover with Map SVG */}
                      <div className="h-32 bg-gray-900 relative overflow-hidden flex-shrink-0">
                        ...
                        <svg className="absolute inset-0 w-full h-full text-white" fill="none" viewBox="0 0 290 150" preserveAspectRatio="none">
                          <path stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" d="M28 110 C 90 70, 130 120, 175 60 S 245 40, 268 28" />
                        ...
                        {/* Saved Tag appears in Step 2 */}
                        <span className={`absolute top-3 right-3 text-[10px] font-bold px-2.5 py-1 rounded-full text-white bg-[#FF6B35] shadow transition-all duration-500 transform ${activeStep === 2 ? "scale-100 opacity-100" : "scale-50 opacity-0"}`}>♥ Saved</span>
                      </div>
                      ...
                      <div className="p-4 flex flex-col justify-between flex-1 text-left">
                        <div>
                          ...
                          <div className="text-sm font-black text-gray-900 mb-2">8 days in Portugal</div>
                          <div className="flex gap-1.5 flex-wrap">
                            <span className="bg-gray-100 px-2 py-0.5 rounded-md text-[9px] font-bold text-gray-600">Day 1</span>
                            <span className="bg-gray-100 px-2 py-0.5 rounded-md text-[9px] font-bold text-gray-600">Day 2</span>
                            <span className="bg-gray-100 px-2 py-0.5 rounded-md text-[9px] font-bold text-gray-600">+5</span>
                          </div>
                        </div>

                        {/* Card Actions (Buttons animated automatically) */}
                        <div className="mt-auto flex flex-col gap-2">
                          <div className="flex gap-2 w-full">
                            <button className={`flex-1 flex items-center justify-center gap-1.5 text-[10px] font-bold border rounded-xl py-2 transition-all duration-300 ${activeStep === 1 ? "bg-[#FF6B35] text-white border-[#FF6B35] shadow-md" : "bg-gray-50 text-gray-500 border-gray-200"}`}>Save</button>
                            <button className={`flex-1 flex items-center justify-center gap-1.5 text-[10px] font-bold rounded-xl py-2 border transition-all duration-300 ${activeStep === 2 ? "bg-[#00293D] text-white border-[#00293D] shadow-md scale-105" : "bg-white text-gray-600 border-gray-200"}`}>Copy</button>
                          </div>
                          {/* Success Alert in Step 2 */}
                          <div className={`text-[9px] font-bold p-2 text-center rounded-xl transition-all duration-500 transform border ${activeStep === 2 ? "bg-[#FF6B35]/10 text-[#FF6B35] border-[#FF6B35]/30 translate-y-0 opacity-100" : "opacity-0 translate-y-2 pointer-events-none"}`}>
        ...
        <div className="relative w-full max-w-7xl mx-auto px-6 overflow-visible">
          ...
          >
            ...
            <div className="flex flex-col gap-6 min-w-[280px] md:min-w-[320px] h-[460px] justify-between snap-center text-left">
              <div className="bg-white rounded-[2rem] p-6 h-[218px] shadow-sm flex flex-col justify-between border border-gray-100">
                <div>
            ...
            <div className="relative rounded-[2rem] min-w-[280px] md:min-w-[320px] h-[460px] shadow-lg overflow-hidden snap-center group text-left">
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-[#000A0F]/90 via-[#000A0F]/20 to-transparent" />
            ...
            <div className="relative rounded-[2rem] min-w-[280px] md:min-w-[320px] h-[460px] shadow-lg overflow-hidden snap-center group text-left">
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-[#000A0F]/90 via-[#000A0F]/20 to-transparent" />
              <div className="absolute top-6 left-6 flex items-center gap-3">
                <div className="w-8 h-8 rounded-full bg-[#00DB9A] flex items-center justify-center text-xs font-bold text-[#001621]">LK</div>
      ...
      <div className="relative bg-[#000A0F] overflow-hidden">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-6 flex flex-col items-center pt-24">
          <h2 className="text-3xl md:text-4xl font-extrabold text-white text-center tracking-tight max-w-[18ch] leading-tight mb-8 drop-shadow-md">
